// user messages.js ilə eyni məntiq
// yalnız courier guard əlavə olunur
...
function threadTemplate(thread) {
  ...
  return `<article class="thread-item ${activeThreadId === thread.id ? "active" : ""}" data-thread-id="${thread.id}">
    <h4>${title}</h4>
...
function messageTemplate(message, myUserId) {
  ...
  return `<article class="message-item ${mine ? "mine" : ""}">
...
(async function initCourierMessages() {
  const ok = await requireRole("courier");
  if (!ok) return;
